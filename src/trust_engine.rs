//! Two-way trust ledger for Kickback.ai.
//!
//! Local Kickback.ai surfaces need to prove two things at once:
...
//! probe traffic, and payout abuse. This module classifies local evidence,
//! permits opt-in official adapter candidates, and names backend-required
//! settlement gates. It does not turn local observations or probes into money.
...
pub struct TrustBoundary {
    ...
    pub can_create_billable_event: bool,
...
pub struct TrustTransitionRule {
    ...
    pub owner: &'static str,
    pub required_evidence: &'static str,
    pub can_be_initiated_by_local_surface: bool,
...
pub struct EventStateBucket {
    ...
    pub can_bill_advertiser: bool,
    pub can_pay_developer: bool,
    pub local_surface_authority: &'static str,
}
...
pub struct SurfaceAttestation {
    ...
    pub can_create_earning_candidates: bool,
    pub can_enter_backend_acceptance_gates: bool,
...
pub struct AdvertiserProtection {
    ...
    pub gross_adapter_events: Option<i64>,
    pub held_events: Option<i64>,
    pub rejected_or_fraudulent_events: Option<i64>,
    pub final_billable_reach: Option<i64>,
    pub payout_releases_after_trust_window: Option<i64>,
...
pub fn current(archive: &Archive) -> Result<TrustEngineSnapshot> {
    ...
    Ok(TrustEngineSnapshot {
        title: "Kickback.ai Trust Engine",
        pitch: "Opt-in official adapters create earning candidates; backend ledgers decide settlement; ML contributes data points, not payout authority.",
        mode: "local trust ledger plus opt-in official adapter candidate contract",
        ...
        advertiser_refund_exposure: ExposureSummary {
            status: if held_for_sync_review > 0 {
                ...
                "local_only_no_backend_billing_claim"
            },
        ...
        payout_hold_queue: QueueSummary {
            ...
            reason: if held_for_sync_review > 0 {
                "Local adapter sends are newer than the visible account ledger watermark; this should trigger account-display reconciliation, not a claim that Stripe delivery failed. Backend-owned billing and payout ledgers own payability after refund-window clearance.".to_string()
            } else {
        ...
        non_earning_probe_mode: ProbeMode {
            ...
            statement: "Install, repair, doctor, skills, dashboard, and Trust Engine flows are probe-only: they can validate setup without creating earning candidates, billing, or payable events.",
            safe_commands: vec![
                "kickbacks doctor",
        ...
        safety_policy: SafetyPolicy {
            earning_integrity_invariant:
                "An opt-in official earning adapter may report candidate impressions, views, or clicks for backend acceptance gates. Local observation, dashboards, diagnosis, repair, and probes remain non-earning.",
            ...
            dashboard_must_not: vec![
                "post unauthenticated metrics",
                "retry earning events outside the official adapter path",
                "hold Stripe secrets",
                "create charges",
...
fn is_official_earning_adapter(item: &integrations::IntegrationStatus) -> bool {
    item.detected && matches!(&item.capability, integrations::IntegrationCapability::Earn)
}
...
fn backend_owned_boundaries() -> Vec<BackendOwnedBoundary> {
    vec![
        ...
        BackendOwnedBoundary {
            ...
            local_snapshot_policy:
                "opt-in official adapters may submit candidate receipts from any approved surface; dashboard, doctor, repair, skills, and trust views must not create, retry, or imply payable/billing events",
        },
        BackendOwnedBoundary {
        ...
        BackendOwnedBoundary {
            ...
            local_snapshot_policy:
                "hold claims at the trust boundary; never let local surface state release or promise funds",
        },
    ]
}
...
fn trust_boundaries() -> Vec<TrustBoundary> {
    vec![
        ...
        TrustBoundary {
            ...
            owner: "Kickback.ai opt-in official earning adapter",
            evidence: vec![
                "user earning opt-in",
                "adapter identity",
                "adapter version",
            ...
            can_make_payable: false,
            failure_mode:
                "An opt-in official adapter can prove a candidate event from an approved surface, but billing and payability still wait for backend caps, attestation, and fraud checks.",
        },
        TrustBoundary {
            stage: "backend_acceptance",
            owner: "Kickback.ai backend trust service",
            evidence: vec![
            ...
            can_make_payable: false,
            failure_mode:
                "Backend acceptance can make an event billable, but it is not payable until refund and payout holds clear.",
        },
        TrustBoundary {
            stage: "refund_window_clearance",
            owner: "Kickback.ai billing and payout ledger",
            evidence: vec![
                "backend_accepted event row",
                "refund buffer elapsed or cleared",
                "payout hold release",
                "KYC/1099/Connect readiness",
            ],
            allowed_to_advance: "accepted_billable_after_refund_window or refunded/rejected/fraudulent",
            can_create_billable_event: false,
            can_make_payable: true,
            failure_mode:
                "Local evidence and backend acceptance alone cannot release payability before billing and payout ledgers clear the trust window.",
        },
        TrustBoundary {
            stage: "advertiser_settlement",
            owner: "Kickback.ai billing/refund ledger",
            evidence: vec![
        ...
        TrustBoundary {
            ...
            owner: "Kickback.ai backend plus Stripe Connect",
            evidence: vec![
...
fn settlement_gates(
    ...
    vec![
        ...
        SettlementGate {
            ...
            required_evidence:
                "Event came from an approved opt-in earning adapter, not dashboard/skill/repair/probe code.",
    ...
    FinalityModel {
        ...
        billing_state_machine: vec![
            ...
            BillingState {
                ...
                meaning: "Adapter claims user opt-in, render/wait-state threshold, official identity, and signed receipt.",
                can_bill_advertiser: false,
                can_pay_developer: false,
                next_allowed: vec![
                    "held_for_review",
        ...
        hard_stop_rules: vec![
            ...
            HardStopRule {
                ...
                final_until: "Stripe/Kickback.ai payout requirements are complete",
            },
        ],
        finality_rules: vec![
            ...
            "Official opt-in adapters create candidates only; dashboard, skills, repair, doctor, and plain bot/status flows are never settlement authorities.",
        ],
    }
}
...
fn trust_transition_rules() -> Vec<TrustTransitionRule> {
    vec![
        TrustTransitionRule {
            from: TrustEventState::Probe,
            to: TrustEventState::Probe,
            owner: "installer / repair / doctor / dashboard / skill",
        ...
        TrustTransitionRule {
            ...
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
        TrustTransitionRule {
        ...
        TrustTransitionRule {
            from: TrustEventState::EligibleButCapped,
            to: TrustEventState::EligibleButCapped,
            owner: "backend cap and velocity engine",
            required_evidence: "cap policy remains in force",
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
        TrustTransitionRule {
            from: TrustEventState::Paid,
            to: TrustEventState::Paid,
            owner: "Stripe Connect payout flow",
            required_evidence: "payout release already recorded",
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
        TrustTransitionRule {
            from: TrustEventState::Refunded,
            to: TrustEventState::Refunded,
            owner: "billing and refund ledger",
            required_evidence: "advertiser credit or refund already recorded",
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
        TrustTransitionRule {
            from: TrustEventState::Rejected,
            to: TrustEventState::Rejected,
            owner: "backend trust service",
            required_evidence: "terminal rejection reason already recorded",
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
        TrustTransitionRule {
            from: TrustEventState::Fraudulent,
            to: TrustEventState::Fraudulent,
            owner: "backend trust service",
            required_evidence: "terminal invalid-traffic reason already recorded",
            can_be_initiated_by_local_surface: false,
            creates_billable_event: false,
            creates_payable_event: false,
        },
...
fn advertiser_assurance_report() -> AdvertiserAssuranceReport {
    AdvertiserAssuranceReport {
        ...
        proof_standard:
            "Show gross, held, rejected/fraudulent, refunded, final billable reach, and payout release separately; never collapse them into one opaque impression count.",
        public_claim:
            "Kickback.ai should filter invalid traffic before payout finality and give advertisers an auditable reason ledger for every non-billable or refunded event.",
        required_tables: vec![
            AssuranceTable {
        ...
        report_metrics: vec![
            ...
            AdvertiserMetric {
                metric: "payout_releases_after_trust_window",
    ...
    vec![
        ThreatModelItem {
            ...
            required_controls: vec![
                ...
                "duplicate rejection",
                "cap ledger",
                "held payout until refund-window clearance",
            ],
        },
        ThreatModelItem {
...
fn ml_signal_layer() -> MlSignalLayer {
    MlSignalLayer {
        ...
        labeled_training_data:
            "Known bot accounts and bot events labeled by Kickback.ai, when available, should bootstrap supervised training, validation, and precision/recall reporting.",
        feature_row: vec![
            "account_age",
            "kyc_status",
            "surface_type",
        ...
        model_layers: vec![
            ...
            ModelLayer {
                ...
                settlement_authority:
                    "signals only; recommends hold/review evidence while policy ledger owns the transition",
            },
            ModelLayer {
...
fn eligibility_buckets(stats: &Stats, held_for_sync_review: usize) -> Vec<EligibilityBucket> {
    vec![
        ...
        EligibilityBucket {
            ...
            payout_impact: "local proof only; not payable from the local archive",
...
fn event_state_buckets(stats: &Stats, held_for_sync_review: usize) -> Vec<EventStateBucket> {
    vec![
        EventStateBucket {
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority: "local probe surfaces can create this bucket only; it never upgrades",
        },
        EventStateBucket {
            state: "observed_local",
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority:
                "local surfaces can count local observations but cannot promote them to earning candidates",
        },
        EventStateBucket {
            state: "candidate_adapter_attested",
            ...
            can_pay_developer: false,
            local_surface_authority:
                "official opt-in adapter may create candidate receipts from an approved surface; backend owns acceptance",
        },
        EventStateBucket {
            state: "held_for_review",
            ...
            can_pay_developer: false,
            local_surface_authority:
                "local surfaces may recommend a hold for visible lag, but backend owns release",
        },
        EventStateBucket {
            state: "eligible_but_capped",
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority: "local surfaces can name cap policy; backend owns cap arithmetic",
        },
        EventStateBucket {
            state: "backend_accepted",
            ...
            can_bill_advertiser: true,
            can_pay_developer: false,
            local_surface_authority: "local surfaces may show backend-provided accepted counts only",
        },
        EventStateBucket {
            state: "accepted_billable_after_refund_window",
            ...
            can_bill_advertiser: true,
            can_pay_developer: true,
            local_surface_authority: "local surfaces may show backend-provided payout-ready counts only",
        },
        EventStateBucket {
            state: "paid",
            ...
            can_bill_advertiser: true,
            can_pay_developer: true,
            local_surface_authority: "local surfaces may display Stripe/backend payout status only",
        },
        EventStateBucket {
            state: "refunded",
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority: "local surfaces may display backend-provided refund status only",
        },
        EventStateBucket {
            state: "rejected",
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority: "local surfaces may display backend-provided rejection reasons only",
        },
        EventStateBucket {
            state: "fraudulent",
            ...
            can_bill_advertiser: false,
            can_pay_developer: false,
            local_surface_authority: "local surfaces may display backend-provided fraud outcomes only",
        },
    ]
}
    ...
    integrations
        ...
        .map(|item| {
            ...
            SurfaceAttestation {
                ...
                can_earn,
                can_observe,
                can_create_earning_candidates: official_adapter,
                can_enter_backend_acceptance_gates: official_adapter,
...
fn cluster_signals(
    ...
    vec![
        ClusterSignal {
            ...
            local_evidence: "not observable from a local archive".to_string(),
        ...
        ClusterSignal {
            ...
            severity: "backend_required",
            local_evidence: "local surfaces do not know account age or actual payout balance"
                .to_string(),
...
fn advertiser_protection() -> AdvertiserProtection {
    AdvertiserProtection {
        proof_statement: "Advertisers should see gross adapter events, held events, rejected/fraudulent events, refunded events, final billable reach, and payout releases as separate backend-owned metrics.",
        billing_counts_backend_owned: true,
        local_counts_are_not_invoiceable: true,
        gross_adapter_events: None,
        held_events: None,
        rejected_or_fraudulent_events: None,
        final_billable_reach: None,
        payout_releases_after_trust_window: None,
...
mod tests {
    ...
    fn trust_engine_names_two_way_ledger_and_probe_mode() {
        ...
        assert_eq!(snapshot.non_earning_probe_mode.event_state, "probe");
        assert!(snapshot
            .trust_boundaries
            .iter()
            .any(|b| b.stage == "backend_acceptance"
                && b.can_create_billable_event
                && !b.can_make_payable));
        assert!(snapshot
            .trust_boundaries
            .iter()
            .any(|b| b.stage == "refund_window_clearance"
                && !b.can_create_billable_event
                && b.can_make_payable));
        assert_eq!(
        ...
        assert!(snapshot
            .ml_signal_layer
            .labeled_training_data
            .contains("labeled by Kickback.ai"));
        assert!(snapshot
            .safety_policy
            .event_states
            .contains(&"held_for_review"));
        assert!(snapshot.safety_policy.event_states.contains(&"probe"));
        assert!(snapshot
            .safety_policy
            .event_states
    ...
    fn local_probe_surfaces_do_not_call_payable_or_stripe_money_routes() {
        ...
        for (surface, source) in surfaces {
            let runtime_source = production_source(source);
            for pattern in forbidden {
                assert!(
                    !runtime_source.contains(pattern),
    ...
    #[test]
    fn backend_acceptance_is_billable_but_refund_window_clearance_owns_payability() {
        let archive = Archive::open_in_memory().unwrap();
        let snapshot = current(&archive).unwrap();
        let backend = snapshot
            .trust_boundaries
            .iter()
            .find(|boundary| boundary.stage == "backend_acceptance")
            .expect("missing backend acceptance boundary");
        assert!(backend.can_create_billable_event);
        assert!(!backend.can_make_payable);

        let refund_window = snapshot
            .trust_boundaries
            .iter()
            .find(|boundary| boundary.stage == "refund_window_clearance")
            .expect("missing refund-window clearance boundary");
        assert!(!refund_window.can_create_billable_event);
        assert!(refund_window.can_make_payable);

        for boundary in snapshot
            .trust_boundaries
            .iter()
            .filter(|boundary| boundary.stage != "backend_acceptance")
        {
            assert!(!boundary.can_create_billable_event);
        }
        for boundary in snapshot
            .trust_boundaries
            .iter()
            .filter(|boundary| boundary.stage != "refund_window_clearance")
        {
            assert!(!boundary.can_make_payable);
        }
        assert!(snapshot
            .settlement_gates
            .iter()
        ...
        for rule in &snapshot.finality_model.transition_rules {
            ...
            if rule.creates_billable_event {
                ...
                assert!(!rule.can_be_initiated_by_local_surface);
            }
        }
        assert!(snapshot
            .threat_model
    ...
    #[test]
    fn official_adapter_candidates_can_enter_backend_acceptance_gates() {
        let archive = Archive::open_in_memory().unwrap();
        let snapshot = current(&archive).unwrap();
        let candidate = snapshot
            ...
            .iter()
            .find(|state| state.state == "candidate_adapter_attested")
            .expect("missing candidate adapter state");
        assert!(!candidate.can_bill_advertiser);
        assert!(!candidate.can_pay_developer);
        assert!(candidate.next_allowed.contains(&"backend_accepted"));
        assert!(candidate.next_allowed.contains(&"held_for_review"));

        let local_candidate_rule = snapshot
            .finality_model
            .transition_rules
            .iter()
            .find(|rule| {
                rule.from == TrustEventState::CandidateAdapterAttested
                    && rule.to == TrustEventState::CandidateAdapterAttested
            })
            .expect("missing local candidate attestation rule");
        assert!(local_candidate_rule.can_be_initiated_by_local_surface);
        assert!(!local_candidate_rule.creates_billable_event);
        assert!(!local_candidate_rule.creates_payable_event);

        let backend_acceptance = snapshot
            .finality_model
            .transition_rules
            .iter()
            .find(|rule| {
                rule.from == TrustEventState::CandidateAdapterAttested
                    && rule.to == TrustEventState::BackendAccepted
            })
            .expect("missing backend acceptance gate");
        assert!(!backend_acceptance.can_be_initiated_by_local_surface);
        assert!(backend_acceptance.creates_billable_event);
        assert!(!backend_acceptance.creates_payable_event);
    }

    #[test]
    fn only_refund_window_finality_can_release_payout() {
        let archive = Archive::open_in_memory().unwrap();
        let snapshot = current(&archive).unwrap();
        for state in &snapshot.finality_model.billing_state_machine {
            if state.can_pay_developer {
                assert!(
                    matches!(
                        state.state,
                        "accepted_billable_after_refund_window" | "paid"
                    ),
                    "unexpected payable state: {:?}",
                    state
                );
            }
            if state.state == "backend_accepted" {
                assert!(!state.can_pay_developer);
            }
        }
    ...
    fn advertiser_assurance_exposes_filtered_counts_and_evidence() {
        ...
        for metric in [
            ...
            "refunded_events",
            "final_billable_reach",
            "payout_releases_after_trust_window",
        ] {
            assert!(
    ...
    #[test]
    fn event_states_are_exact_and_transition_targets_are_declared() {
        let archive = Archive::open_in_memory().unwrap();
        let snapshot = current(&archive).unwrap();
        ...
        assert_eq!(snapshot.safety_policy.event_states, expected_states);
        let declared: HashSet<&str> = snapshot
            ...
            .copied()
            .collect();
        let transitions: HashSet<(&str, &str)> = snapshot
            .finality_model
            .transition_rules
            .iter()
        ...
        for state in &snapshot.finality_model.billing_state_machine {
            ...
            for next in &state.next_allowed {
                ...
                assert!(
                    transitions.contains(&(state.state, *next)),
                    "missing explicit transition {} -> {next}",
                    state.state
                );
            }
        }
        for rule in &snapshot.finality_model.transition_rules {
            let state = snapshot
                ...
                .billing_state_machine
                .iter()
                .find(|state| state.state == rule.from.as_str())
                .unwrap_or_else(|| panic!("missing transition source {}", rule.from.as_str()));
            assert!(
                state.next_allowed.contains(&rule.to.as_str()),
    ...
    fn local_and_probe_transitions_cannot_become_paid() {
        ...
        let snapshot = current(&archive).unwrap();
        for rule in &snapshot.finality_model.transition_rules {
            if rule.can_be_initiated_by_local_surface
                || matches!(
                    rule.from,
                    TrustEventState::Probe | TrustEventState::ObservedLocal
                )
            {
        ...
        for state in ["probe", "observed_local"] {
            let bucket = buckets
                .iter()
            ...
            assert!(
                bucket.local_surface_authority.contains("cannot")
                    || bucket.local_surface_authority.contains("never")
            );
        }
    }

    #[test]
    fn docs_match_exact_state_set_and_backend_authority() {
        let docs = include_str!("../TRUST_ENGINE.md");
        for state in event_state_names() {
            assert!(
                docs.contains(&format!("| `{state}` |")),
                "docs missing state {state}"
            );
        }
        for obsolete in [
            "| `probe_non_earning` |",
            "| `credited` |",
            "| `reversed` |",
            "`payouts_released_after_trust_window`",
        ] {
            assert!(!docs.contains(obsolete), "obsolete doc claim {obsolete}");
        }
        assert!(docs.contains("Local observations are evidence"));
        assert!(docs.contains("backend-owned certainty"));
    }

    #[test]
    fn ml_layers_are_signals_not_settlement_authority() {
        let archive = Archive::open_in_memory().unwrap();
        let snapshot = current(&archive).unwrap();
        assert!(snapshot
            .ml_signal_layer
            .authority_boundary
            .contains("state machine decide settlement"));
        for layer in &snapshot.ml_signal_layer.model_layers {
            let authority = layer.settlement_authority.to_ascii_lowercase();
            assert!(
                authority.contains("cannot")
                    || authority.contains("signals only")
                    || authority.contains("review queues")
                    || authority.contains("evidence"),
                "{layer:?}"
            );
            assert!(
                !authority.contains("can pay")
                    && !authority.contains("can reject")
                    && !authority.contains("settles"),
                "{layer:?}"
            );
        }
    }
